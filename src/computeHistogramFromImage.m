...
%   COMPUTEPHOW(), QUANTIZEFEATURES(), and COMPUTEHISTOGRAM().
%
...
width = size(im,2) ;
height= size(im,1) ;
[keypoints, descriptors] = computePHOW(im);
words = quantizeDescriptors(vocabulary, descriptors) ;
histogram = computeHistogram(width, height, keypoints, words, vocabulary);
